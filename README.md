...
每多一次手动操作，就是一次设计失败。自动化部分应该由 Agent 一次性处理完再找用户。
...
### ⑤ 备用方案以用户最小感知量和正确输出为第一顺位

主方案不可用时，备用方案的设计目标是：**用户感知最小化，输出正确性优先。**

用户不需要知道主方案挂了，模型默默兜底，输出正确结果，末尾提一句即可。

```
✅ 正确：
  用户: "搜一下 Rust vs Go 的讨论"
  模型: 调用 X MCP → Cookie 失效
        → 自动用搜索引擎搜 site:twitter.com "Rust vs Go"
        → 输出结果
        → 末尾提一句 "X 登录过期了，有空重新登录一下"

❌ 错误：
  用户: "搜一下 Rust vs Go 的讨论"
  模型: "X Cookie 无效，无法搜索。你要不要重新登录？"
```

### ⑥ 避免模型执行无意义操作

模型遇到错误时倾向于自行修复（重试命令、改参数、试其他方法），但通常无效且浪费大量时间、token 和上下文空间。

**明确禁止模型自行修复。** 告诉它：主方案失败 → 执行备用方案 → 仍不行 → 如实告知用户，跳过。

```
✅ 正确：
  主方案失败 → 备用方案兜底 → 仍不行 → "暂时获取不到，跳过"

❌ 错误：
  主方案失败 → 重试 3 次 → 换参数 → 换个工具 → 再失败 → 报告用户
```
